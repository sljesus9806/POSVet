# Resultados de pruebas funcionales — POSVet

> Ejecutado: 2026-05-30T08:29:58.562Z
...
|----|------|-----------|------|
| CFG-01 | obtenerEmpresaPrincipal devuelve la empresa del seed | ✅ | POSVet Veterinaria Demo SA de CV |
...
| CFG-04 | crearUbicacion Sucursal Centro (SUCURSAL) creada y activa | ✅ | cmps3bfsv0002wolkwjhwh43y |
...
| USR-02 | crear un usuario de cada rol (CAJERO/ALMACENISTA/SUPERVISOR/READONLY) | ✅ | CAJERO creado (cmps3bg040007wolkevflx7q8) |
| USR-02 | crear un usuario de cada rol (CAJERO/ALMACENISTA/SUPERVISOR/READONLY) | ✅ | ALMACENISTA creado (cmps3bg6w000awolkyzvb001t) |
| USR-02 | crear un usuario de cada rol (CAJERO/ALMACENISTA/SUPERVISOR/READONLY) | ✅ | SUPERVISOR creado (cmps3bgds000dwolkorcabi9b) |
| USR-02 | crear un usuario de cada rol (CAJERO/ALMACENISTA/SUPERVISOR/READONLY) | ✅ | READONLY creado (cmps3bgkj000gwolkhbqxwbzw) |
...
| USR-07 | 5 intentos fallidos → bloqueo 15 min (UsuarioBloqueadoError con pass correcta) | ✅ | bloqueadoHasta=2026-05-30T08:44:57.465Z |
...
| PRD-02 | crear medicamento (IVA, receta, controlada, lab, vía) + precios PUBLICO/MAYOREO/VETERINARIO | ✅ | 3 listas de precio |
| PRD-02 | crear medicamento (IVA, receta, controlada, lab, vía) + precios PUBLICO/MAYOREO/VETERINARIO | ✅ | flags receta/controlada |
...
| ID | Caso | Resultado | Nota |
|----|------|-----------|------|
| CLI-01 | crear PUBLICO, MAYOREO, VETERINARIO (con crédito) y GRANJA | ✅ | VETERINARIO con línea 5000 |
| CLI-01 | crear PUBLICO, MAYOREO, VETERINARIO (con crédito) y GRANJA | ✅ | tipo de precio efectivo VETERINARIO |
| CLI-01 | crear PUBLICO, MAYOREO, VETERINARIO (con crédito) y GRANJA | ✅ | GRANJA mapea a precio MAYOREO |
| CLI-01 | crear PUBLICO, MAYOREO, VETERINARIO (con crédito) y GRANJA | ✅ | PUBLICO sin crédito |
...
| CLI-03 | listar con filtro por tipo y obtener | ✅ | filtro tipo VETERINARIO (2) |
...
| VEN-07 | crearVenta a crédito → saldoCredito venta + saldoActual cliente incrementados | ✅ | precio VETERINARIO de iver = 155 |
...
| VEN-08 | precio por tipo de cliente (MAYOREO) aplica la lista correcta | ✅ | precio MAYOREO del alimento canino = 820 |
...
| VEN-09 | pago insuficiente / cambio no-efectivo / stock insuficiente / crédito excede / sin precio | ✅ | stock insuficiente (felino) |
...
| REP-09 | generarReportePDF produce un buffer válido para cada reporte | ✅ | 8 PDFs generados (ventas-dia:6010B, productos-vendidos:7393B, ventas-por-usuario:4961B, inventario-actual:14920B, por-caducar:5439B, antiguedad-cxc:4958B, antiguedad-cxp:4974B, sin-movimiento:6776B) |
...
| ID | Caso | Resultado | Nota |
|----|------|-----------|------|
| CON-01 | concurrencia: 2 ventas simultáneas del mismo producto con stock para una | ✅ | exitosas=1, stock final=0 · colisión de folio en la concurrente |

## Bugs encontrados

### [MEDIA] Generación de folio de venta no es concurrency-safe (colisión bajo carga)  (CON-01)

Dos crearVenta simultáneas → 1 venta exitosa y la otra abortó por colisión de folio único: PrismaClientKnownRequestError: 
Invalid `tx.venta.create()` invocation in
/home/jsalazar/POSVet/src/lib/modules/ventas/service.ts:500:32

  497 // Transacción: crear venta + líneas + pagos + descontar inventario atómicamente
  498 const { venta, alertasBajoStock } = await prisma.$transaction(async (tx) => {
  499   const folio = await ventasRepository.proximoFolioVenta(tx);
→ 500   const v = await tx.venta.create(
Unique constraint failed on the fields: (`folio`). proximoFolioVenta calcula el consecutivo con un COUNT/MAX y dos transacciones concurrentes obtienen el mismo folio; la unique constraint protege la integridad pero la venta falla con un error técnico (no un error de negocio). En esta corrida la colisión evitó la sobreventa de stock (solo 1 transacción llegó a descontar). Mitigación: secuencia/serial de Postgres para el folio, o reintento ya presente en otros módulos. Ver [[posvet-estado-y-mejoras]].
