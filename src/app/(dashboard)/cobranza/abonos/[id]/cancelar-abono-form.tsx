...
import { cancelarAbonoAction, type FormState } from "../../actions";
...
export function CancelarAbonoForm({ abonoId, folio }: { abonoId: string; folio: string }) {
  const [state, action] = useActionState(cancelarAbonoAction, initial);
  const [mostrar, setMostrar] = useState(false);

  if (!mostrar) {
    return (
      <Button variant="outline" type="button" onClick={() => setMostrar(true)}>
        <Ban className="size-4" /> Cancelar abono
      </Button>
    );
  }
