...
export function CancelarFacturaForm({
  facturaId,
  folio,
}: {
  facturaId: string;
  ...
  if (!mostrar) {
    return (
      <Button variant="outline" type="button" onClick={() => setMostrar(true)}>
        <Ban className="size-4" /> Cancelar factura
      </Button>
    );
  }

  return (
    <form action={action} className="w-full rounded-md border bg-muted/30 p-4 space-y-3">
      ...
      <div>
        ...
        <Input id="motivo" name="motivo" required minLength={3} placeholder="Ej: nota de crédito" />
      </div>
