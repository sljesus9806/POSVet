...
export default async function NuevoProductoPage() {
  ...
  const [categorias, ubicaciones, productos] = await Promise.all([
  ...
  const productosCostal = productos.map((p) => ({ id: p.id, nombre: p.nombre, sku: p.sku }));
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <ProductoForm categorias={categorias} ubicaciones={ubicaciones} productosCostal={productosCostal} />
    </div>
  );
}
